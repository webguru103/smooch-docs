...
Recommending content to your users is a great way to share important information, and solve users' problems before they have to tap the "Messages" button. Smooch beautifully previews your recommendations, so users can easily browse and find the one they are looking for. Content can be from any URL including online videos like Youtube.

![Recommended answers](/images/recommended.png)

The Smooch SDK provides two types of recommendations -  a static default list of recommendations, and one dynamic top recommendation.
...
```objective_c
[Smooch setDefaultRecommendations:@[ @"https://www.somearticle.com/article", @"https://somevideotoshow.com" ]];
```
```swift
Smooch.setDefaultRecommendations(["https://www.somearticle.com/article", "https://somevideotoshow.com"])
```

The top recommendation will appear at the beginning of the list and will be the first one the user sees upon launching Smooch. This recommendation should attempt to solve a problem the user has just experienced so they don't need to search for it. For example, if a user fails to log in, you may choose to recommend an article explaining the various types of login errors that can occur, and how to fix and/or work around them.

Use the `setTopRecommendations:` API to set these, for example: 

```objective_c
[Smooch setTopRecommendation:@"https://yourdomain.zendesk.com/hc/en-us/articles/000000000-context-driven-recommendation"];
```
```swift
Smooch.setTopRecommendation("https://yourdomain.zendesk.com/hc/en-us/articles/000000000-context-driven-recommendation")
```
