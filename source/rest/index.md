...
```shell
curl https://sdk.supportkit.io/v1/init \
     -X POST \
...
| **userId**<br/>*optional* | A unique identifier for the app user. Unlike the `appUserId` which is generated by SupportKit, the `userId` is chosen by the API consumer. The `userId` can be used to link a user to the same conversation across multiple devices. [More information](/#users-on-multiple-devices)|
...
Provided `userId`s are specified in the call to `/v1/init`, devices and app users have a many-to-many relationship. A single `userId` may use SupportKit from multiple devices and multiple different `userId`s may log in to the same device. Note however that only one `userId` can be active on a given device at a time. That is, if a device is shared among multiple `userId`s, only the most recent `userId` to have called `/v1/init` will receive push notifications from the integrated application on that device.
