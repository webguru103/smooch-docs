...
Sending images to users is simple and supported through both [messaging shorthand](/docs/sending-images-and-buttons-shorthand/) and the API.

In order to send an image using the API, we follow the recipe for [sending text messages](/docs/sending-messages/), but set the `type` parameter to `image` and set the `mediaUrl` field to a URL for the image to be sent to the user.
...
Buy buttons allow you to collect payments from your users. To learn more about how this works, read about our [payments integration](/docs/stripe-payments/)
...
#### Carousel Messages
Carousel messages are a horizontally scrollable set of items that may each contain text, an image, and action buttons. Not all messaging channels fully support carousel messages; to check the level of support across Smooch messaging channels see [channel feature grid](/docs/channel-capabilities).
 ...
 * Consult the [channel capabilities grid](/docs/channel-capabilities) to understand how your structured message will be displayed across different platforms.
 * Send your first structured message by using [messaging shorthand](/docs/sending-images-and-buttons-shorthand).
